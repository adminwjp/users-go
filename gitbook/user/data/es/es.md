...
import (
	"context"
	"github.com/adminwjp/infrastructure-go/data/es"
...
func(admin *AdminDaoImpl) Login(user dto.UserInput)*model.AdminModel{
	...
	log.Printf("admin es account login: account %s ",user.Account)
	...
	_, err := admin.Client.Search("admin").Query(nil).
		Source(&adminModel).Do(context.Background())
	if err!=nil{
		log.Printf( "admin es account login fail: account %s,err => %s",
