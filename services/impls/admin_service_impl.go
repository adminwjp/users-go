...
type AdminServiceImpl struct {
	BaseUserServiceImpl
	Dao	daos.AdminDao
  	Cache caches.AdminCache
	LogDao daos.AdminLogDao
...
func (service *AdminServiceImpl)Clean()  {
	//admin
...
根据手机号、邮箱、用户名登录
*/
func (service *AdminServiceImpl)Login(input *dtos.UserInput)(*models.AdminModel,error){
...
根据手机号登录
*/
func (service *AdminServiceImpl)LoginByPhone(input *dtos.UserPhoneInput)(*models.AdminModel,error){
...
根据邮箱登录
*/
func (service *AdminServiceImpl)LoginByEmail(input *dtos.UserEmailInput)(*models.AdminModel,error){
...
根据用户名登录
*/
func (service *AdminServiceImpl)LoginByUserName(input *dtos.UserUserNameInput)(*models.AdminModel,error){
...
/*
	根据旧密码修改密码
*/
func (service *AdminServiceImpl)UpdatePwdByOldPwd(input *dtos.UpdateUserPwdByPwdInput)(int,error){
	r,err:= service.Dao.UpdatePwdByOldPwd(input.Pwd,input.NewPwd)
	if err!=nil{
		log.Printf("%s %s  update pwd fail: pwd %s,err => %s",
			service.user,service.dataWay,input.Pwd,err.Error())
		return r, err
	}
	if r>0{
		log.Printf("%s %s   update pwd  suc: pwd %s",
			service.user,service.dataWay,input.Pwd)
	}else{
		log.Printf("%s %s   update pwd  fail: pwd %s",
			service.user,service.dataWay,input.Pwd)
		return r, err
	}
	_,err= service.Cache.UpdatePwdByOldPwd(input)
	if err!=nil{
		log.Printf("%s %s cache  update pwd  fail: pwd %s,err => %s",
			service.user,service.cache1,input.Pwd,err.Error())
	}
	return r, err
}
...
/*
	根据条件查询用户信息
*/
func (service *AdminServiceImpl)List(user *dtos.GetAdminInput) ([]models.AdminModel,int64,error){
...
/*
	根据id查询用户信息
*/
func (service *AdminServiceImpl)Get(id int64) (*models.AdminModel,error){
...
/*
	根据条件查询用户日志信息
*/
func (service *AdminServiceImpl)ListLogs(user *dtos.GetUserLogInput) ([]models.AdminLogModel,int64,error){
